...
const HUB = [540, 1100];
const M_ST = [[200, 500, 'HDR Photos', 'next-day delivery', 'R'], [200, 780, 'Video', '10+ years behind the lens', 'R'],
  [880, 1380, 'Drone', 'FAA-certified', 'L'], [880, 1520, '3D Tours', '', 'L'], [880, 1660, 'Virtual Staging', '', 'L']];
const S_ST = [[140, 1640, 'Newsletter', 'weekly brief', 'R'], [140, 1460, 'TikTok', '@realtampabay', 'R'],
  [940, 720, 'Instagram', '@realtampabay.dev', 'L'], [940, 520, 'YouTube', '@realtampabay', 'L']];
...
// ---------- scenes ----------
// 0–3.2 asleep on the horizon, a drone buzzes in, power-up
function sWake(t) {
  hdPaper();
  ...
  drawMini('rtb', 540, gy - jump, 12, { blink: t < 1.9 });
  if (t < 1.9) zzz(640, gy - 190, t);
  const dx = lerp(1250, 800, E.out(prog(t, .9, 1.8))), dy = 900 + Math.sin(t * 9) * 12;
  drawMini('drone', dx, dy, 7);
  text(typed(HOOK, prog(t, 2.2, 3.0)), W / 2, 1500, { font: HD.script, size: 58, color: INK, jit: false });
  zoomLines(prog(t, 2.9, 3.2), INK, W / 2, gy - 100);
}
// 3.2–6 dark title card
function sIntro(t) {
  hdDark();
  fadeIn(t);
  ...
  TAGLINE.forEach((s, i) => text(s, W / 2, 1270 + i * 70, { font: HD.script, size: 56, color: '#f6ef9a', alpha: prog(t, 1.4 + i * .3, 1.8 + i * .3), jit: false }));
  void x0;
}
// 6–12 WORLD 1-1: the media line draws, the mascot rides it
function sMedia(t) {
  hdPaper(); fadeIn(t);
  tag('WORLD 1-1');
  heading('The Media Line', 'for agents, Airbnb hosts & developers', t);
  const p = E.inOut(prog(t, .4, 5.0));
  mediaMap(p);
  const da = prog(t, 2.9, 3.3), dr = [965, 1330 + Math.sin(t * 8) * 10];
  if (da > 0) { drawMini('drone', dr[0], dr[1], 5, { alpha: da }); nameTag('DRONE', dr[0], dr[1] + 26, da); }
  const [hx, hy] = headAt(MEDIA, p);
  drawMini('rtb', hx, hy - 12 - Math.abs(Math.sin(t * 10)) * 10, 5);
}
// 12–15 WORLD 1-2: no sq-ft fees, $100 referral credit
function sPrice(t) {
  hdPaper(); fadeIn(t);
  tag('WORLD 1-2');
  ...
  horizon(1560, 330);
  drawMini('rtb', 540, 1560 - Math.abs(Math.sin(prog(t, 2.2, 2.8) * Math.PI)) * 60, 8);
  wipe(prog(t, 2.8, 3.0), HD.tag, 'left');
}
// 15–16.5 WORLD 2 level card
function sWorld2(t) {
  ctx.save(); ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.fillStyle = HD.tag; ctx.fillRect(0, 0, W, H); ctx.restore();
  const k = E.back(prog(t, 0, .45));
  if (k <= 0) return;
  ctx.save(); ctx.translate(W / 2, H / 2); ctx.scale(k, k);
  rc.rectangle(-440, -170, 880, 340, ro(410, { stroke: INK, strokeWidth: 6, fill: Y, fillStyle: 'solid', roughness: .6 }));
...
// 16.5–21 WORLD 2-1: the story line crosses the media line at Tampa Bay
function sStory(t) {
  hdPaper(); fadeIn(t);
  tag('WORLD 2-1');
  heading('The Story Line', 'your weekly brief on Tampa Bay', t);
  routeLine(MEDIA, 1, 3000, Y);
  M_ST.forEach((s, i) => station(s, 1, 3010 + i, .35));
  const p = E.inOut(prog(t, .3, 3.5)), fh = fracAt(STORY, ...HUB);
  ...
  drawMini('pelican', 610, 1290 - pj, 5, { alpha: .4 + .6 * pa }); nameTag('PELICAN', 610, 1316, .4 + .6 * pa);
  const [hx, hy] = headAt(STORY, p);
  drawMini('streetcar', hx, hy - 10, 5);
}
// 21–23.5 HI-SCORE board
function sScore(t) {
  hdDark();
  ...
  if (t > 1.5 && Math.floor(T * 4) % 2) text('IG JUST HIT 12K!', W / 2, 1360, { font: HD.pix, size: 34, color: HD.light, jit: false });
}
// 23.5–25.5 WORLD 2-2: readers stay (hatched vs solid)
function sBars(t) {
  hdPaper(); fadeIn(t);
  tag('WORLD 2-2');
...
// 25.5–27.3 torn paper into the full-bleed brand yellow
function sYellow(t) {
  const tp = prog(t, 0, .55);
  if (tp < 1) { sBars(2); paperTear(tp, Y); return; }
  ctx.save(); ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.fillStyle = Y; ctx.fillRect(0, 0, W, H); ctx.restore();
  const s = prog(t, .55, 1.0);
  kwords("Tampa Bay's story,", W / 2, 820, s, { font: HD.serif, weight: 600, size: 92, color: INK });
  kwords('told every week.', W / 2, 940, prog(t, .8, 1.2), { font: HD.serif, weight: 600, size: 92, color: INK });
  text('development · business · growth', W / 2, 1060, { font: HD.script, size: 50, color: INK, alpha: prog(t, 1.1, 1.4), jit: false });
  sketch([[120, 1400], [540, 1404], [960, 1398]], 1, 710, { stroke: INK, strokeWidth: 2.4, roughness: .8 });
  drawMini('rtb', 540, 1400 - Math.abs(Math.sin(t * 7)) * 40, 7);
}
// 27.3–30 ending
function sEnd(t) {
  hdPaper();
  ...
  text('Book a shoot  ·  Get the weekly brief', W / 2, 1020, { font: HD.sans, weight: 700, size: 40, color: INK, alpha: prog(t, .7, .9), jit: false });
  text('realtampabay.com     @realtampabay', W / 2, 1100, { font: HD.mono, size: 34, color: INK, alpha: prog(t, .85, 1.05), jit: false });
  const gy = 1560; horizon(gy, 820);
  const cast = [['crane', 170, 6], ['pelican', 360, 5], ['rtb', 560, 8], ['streetcar', 820, 5]];
  cast.forEach(([m, x, sc], i) => {
    const a = prog(t, .9 + i * .12, 1.1 + i * .12), hop = Math.sin(prog(t, .9 + i * .12, 1.3 + i * .12) * Math.PI) * 50;
    drawMini(m, x, gy - hop, sc, { alpha: a });
    if (m !== 'rtb') nameTag(MINIS[m].name, x, gy + 32, a);
  });
